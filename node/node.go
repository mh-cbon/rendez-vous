...
func (r *rendezVousNode) PacketConn() *libutp.Socket {
	r.l.Lock()
	defer r.l.Unlock()
	return r.ln.(*libutp.Socket)
}

func (r *rendezVousNode) Start() error {
	r.l.Lock()
	defer r.l.Unlock()
	ln, err := libutp.NewSocket("udp", r.listen)
	if err != nil {
		return err
	}
	r.ln = ln
	r.socket = socket.FromConn(r.ln.(*libutp.Socket))
