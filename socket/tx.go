...
func (t *Tx) Query(data []byte, remote net.Addr, h ResponseHandler) error {
	...
	logger.Infof("%-16v snd %-16v: txid=%-6v len=%-4v %q", t.LocalAddr(), remote, txID, len(data), string(data[1+binary.MaxVarintLen16:]))
...
func (t *Tx) Listen(queryHandler TxHandler) error {
	...
	return t.UDP.Listen(func(data []byte, remote net.Addr) error {
		...
		if kind == "q" {
			logger.Infof("%-16v rcv %-16v: txid=%-6v len=%-4v %q", t.LocalAddr(), remote, txID, len(data)+1+binary.MaxVarintLen16, string(data))
			if queryHandler == nil {
				return nil
			}
